  ...
  },
  {
    ...
    recap: {
      ...
      sections: {
        ...
        mechanisms: [
          { name: "Temporal coherence", where: "Coherence Animator setup", observed: "Participants created short hand-drawn animations under two reference conditions. The activity made continuity across frames easier to inspect and served as a teaching analogy for one coherence problem, not a simulation of a particular video model." },
          { name: "Reference conditions", where: "Run A vs Run B", observed: "Run A let participants see only the previous drawing; Run B added the fixed first drawing as an anchor. With more to refer back to, participants held some background elements steadier but also had more to track." },
          { name: "Keyframe interpolation", where: "A participant's start-and-end constraint", observed: "In a share attributed to Sharleen in the transcript, fixing a start and end frame led the participant to rush the in-between frames toward the endpoint. That prompted a facilitator hypothesis — to check against documented workflows, not shown by the activity — that some systems rely on keyframes and interpolation, which could over-converge and flatten detail." },
        ...
        patterns: [
          { what: "With only the previous frame (Run A), participants made more vivid, active choices; with an anchor frame (Run B), some drifted toward replicating the opening — Yunseo compared Run B to a near-static 'Wimpy Kid' diary.", suggests: "In this human-drawing activity, the added anchor sometimes steadied details but also constrained how much participants changed.", next: "Ask what a generated sequence should be free to change versus must keep fixed, then check how documented workflows manage those constraints." },
        ...
        next: {
          ...
          asyncRoute: {
            prompt: "Choose one small fieldwork route: inspect a sequence, translate an activity for learners, critique or refuse a claim, or bring a rough build to the optional August Showcase. Fill out the feedback form, including whether you would present and which August date works.",
